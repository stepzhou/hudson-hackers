...
Foursquare.prototype.makeRequest = function(query, callback) {
    var newQuery = this.apiUrl + '/v2/' + query + ((query.indexOf('?') > -1) ? '&' : '?') + "client_id=" + this.apiKey + "&client_secret=" + this.secretKey + "&callback=?";
    ...
    // print link - works for spaces as well
    console.log("Link: " + newQuery);
...
Foursquare.prototype.searchVenues = function(lat, lng, venues, callback) {
    this.makeRequest('venues/search?ll=' + lat + ',' + lng + "&query=" + venues,
...
Foursquare.prototype.searchNearVenues = function(location, venues, callback) {
    this.makeRequest('venues/search?near=' + location + "&query=" + venues, 
                     function(response) { callback(response['response']['groups'][0]['items']) });
};
